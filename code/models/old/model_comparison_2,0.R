...
datasets <- list(
  ...
  evenness  = arkaute %>% filter(!is.na(Y_zipf), Y_zipf < -1e-5) %>% mutate(Y_zipf = Y_zipf * -1),
...
for (var_key in names(datasets)) {
  ...
  for (cand in glmm_candidates) {
    ...
    f_formula <- as.formula(paste(col_name, "~ treatment + (1 | sampling)"))    #############################CAMBIO DE PLOT A SAMPLING COMO FACTOR ALEATORIO####################################
    ...
    fit <- tryCatch({
      suppressWarnings({
        if (cand$type == "standard") {
          f_formula <- as.formula(paste(col_name, "~ treatment + s(sampling, bs = 're')")) #############################CAMBIO DE PLOT A SAMPLING COMO FACTOR ALEATORIO####################################
